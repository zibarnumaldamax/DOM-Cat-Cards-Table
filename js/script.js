...
function newCell(tr){
    ...
    nameField.innerHTML = "Имя: " + element.name;

    //Фото
    let photoField = document.createElement("img");
    photoField.src = element.img_link;
    photoField.alt = "Нет изображения";
    photoField.height = "300";

    //Возраст
    let ageField = document.createElement("p");
    ageField.innerHTML = "Возраст: " + element.age;

    //Рейтинг
    let rateField = document.createElement("p");
    rateField.innerHTML = "Рейтинг: " + element.rate;

    //Избранное
    let favouriteField = document.createElement("label");
    let favouriteFieldCheckBox = document.createElement("input");
    favouriteField.innerHTML = "Избранное: ";
    favouriteFieldCheckBox.type = "checkbox";
    favouriteFieldCheckBox.checked = element.favourite;

    //Описание
    let descriptionField = document.createElement("p");
    descriptionField.innerHTML = element.description;

    //id
    catCard.id = "cat" + element.id;
    let idField = document.createElement("p");
    idField.innerHTML = "ID: " + element.id;
